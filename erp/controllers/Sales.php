...
class Sales extends MY_Controller
{
	...
	function index($warehouse_id = NULL)
    {
        ...
        $arr1 = [];
        $i = 0; $j = 0;
        $sql = "select transaction_id, product_id,transaction_type from erp_purchase_items
        where transaction_type = 'sale'";
        $results = $this->db->query($sql)->result();
		$delete_count = 0;
        foreach ($results as $result){//$this->erp->print_arrays($result);
            $sql1 = "SELECT erp_sale_items.id from erp_sale_items
            where erp_sale_items.id = {$result->transaction_id}";
            $results1 = $this->db->query($sql1)->result();
			
            if($results1){
                $arr[$i] = array('result' => $result->transaction_id,'product_id' => $result->product_id,'transaction_type'=>$result->transaction_type);
                $i++;
            }else{
                $arr1[$j] = array('result' => $result->transaction_id,'product_id' => $result->product_id,'transaction_type'=>$result->transaction_type);
                $j++;
				/*
				if($this->db->delete('erp_purchase_items',array('transaction_type'=>'SALE','transaction_id'=>$result->transaction_id,'product_id'=>$result->product_id))){
						if($this->db->delete('erp_inventory_valuation_details',array('type'=>'SALE','field_id'=>$result->transaction_id,'product_id'=>$result->product_id))){
						$delete_count ++;
					}
				}
				*/
		...
		$status = "";
		if($arr_size == $delete_count){
			$status = "success";
		}else{
			$status = "fail";
		}
		//$this->erp->print_arrays($arr1);
		//$this->erp->print_arrays("number of products = ", $arr_size, "| delete_counted = ", $delete_count, "| status " , $status);
	...
	function getListSale($sale_id = null)
    {
	...
	function edit_return($id = NULL)
    {
        ...
        if ($this->form_validation->run() == true && $this->sales_model->returnSale($data, $products, $payment)) {
            ...
            foreach ($inv_items as $item) {
				...
				$returned = $this->sales_model->getReturnedQty($inv->sale_id, $item->product_id, $item->id);
                if (!$row) {
                    $row = json_decode('{}');
                    $row->tax_method = 0;
                    $row->quantity = 0;
            ...
            $this->data['payment_ref'] = $this->site->getReference('sp', $inv->biller_id);
			$this->data['tax_rates'] = $this->site->getAllTaxRates();
			$this->data['setting'] = $this->site->get_setting();
			$this->data['bankAccounts'] =  $this->site->getAllBankAccounts();
        ...
        if ($this->form_validation->run() == true) {
            ...
            if ($sale_order_id){
                ...
                foreach ($items as $item) {
                    ...
                    if ($row->tax_rate) {
                        $tax_rate = $this->site->getTaxRateByID($row->tax_rate);
                        $pr[$ri] = array('id' => $c, 'item_id' => $row->id, 'label' => $row->name . " (" . $row->code . ")", 'row' => $row, 'combo_items' => $combo_items, 'tax_rate' => $tax_rate, 'options' => $options, 'expdates'=>$expdates, 'makeup_cost' => 0,'group_prices'=>$group_prices,'customer_percent' => $customer_group->percent, 'all_group_price' => $all_group_prices);
                    } else {
                        $pr[$ri] = array('id' => $c, 'item_id' => $row->id, 'label' => $row->name . " (" . $row->code . ")", 'row' => $row, 'combo_items' => $combo_items, 'tax_rate' => false, 'options' => $options, 'makeup_cost' => 0, 'expdates'=>$expdates, 'customer_percent' => $customer_group->percent,'group_prices'=>$group_prices, 'all_group_price' => $all_group_prices);
                    }
                    $c++;
                }
			...
			if($quote_ID){
                ...
                foreach ($items as $item) {
                    ...
                    if ($row->tax_rate) {
                        $tax_rate = $this->site->getTaxRateByID($row->tax_rate);
                        $pr[$ri] = array('id' => $c, 'item_id' => $row->id, 'label' => $row->name . " (" . $row->code . ")", 'row' => $row, 'combo_items' => $combo_items, 'tax_rate' => $tax_rate, 'options' => $options, 'expdates'=>$expdates, 'makeup_cost' => 0,'group_prices'=>$group_prices, 'all_group_price' => $all_group_prices);
                    } else {
                        $pr[$ri] = array('id' => $c, 'item_id' => $row->id, 'label' => $row->name . " (" . $row->code . ")", 'row' => $row, 'combo_items' => $combo_items, 'tax_rate' => false, 'options' => $options, 'expdates'=>$expdates, 'makeup_cost' => 0,'group_prices'=>$group_prices, 'all_group_price' => $all_group_prices);
                    }
                    $c++;
                }
	...
	function print_old($id = NULL, $view = NULL, $save_bufffer = NULL)
    {
        $this->erp->checkPermissions('add', true, 'sales');
    ...
    function edit($id = NULL)
    {
        ...
        if ($this->form_validation->run() == true) {
			...
			$reference_no_h = $this->input->post('reference_no_h');
			
			if($reference != $reference_no_h){
				$reference_no = $this->site->getSaleByRef($reference);
				if($reference_no){
					$this->session->set_flashdata('error', lang("duplicate reference number"));
					redirect($_SERVER["HTTP_REFERER"]);
				}
			}
            ...
            for ($r = 0; $r < $i; $r++) {
                ...
                $expire_date_id = isset($_POST['expdate'][$r]) && $_POST['expdate'][$r] != 'false' ? $_POST['expdate'][$r] : null;
				$expdate = $this->sales_model->getPurchaseItemExDateByID($expire_date_id)->expiry;
				//$this->erp->print_arrays($expire_date_id."/",$expdate);
				$real_unit_price = $this->erp->formatDecimal($_POST['real_unit_price'][$r]);
                $unit_price = $this->erp->formatDecimal($_POST['unit_price'][$r]);
				$net_price = $this->erp->formatDecimal($_POST['net_price'][$r]);
                $item_quantity = $_POST['quantity'][$r];
                ...
                if (isset($item_code) && isset($real_unit_price) && isset($unit_price) && isset($item_quantity)) {
                    ...
                    if (isset($item_discount)) {
                        ...
                        if ($dpos !== false) {
                            $pds = explode("%", $discount);
                            $pr_discount = $this->erp->formatDecimal((($unit_price * (Float) ($pds[0])) / 100), 4);
                        } else {
                            $pr_discount = $discount/$item_quantity;
                        }
                    }
            ...
            if (!empty($_POST['val'])) {
                ...
                if ($this->input->post('form_action') == 'export_excel' || $this->input->post('form_action') == 'export_pdf') {
                    ...
                    foreach ($_POST['val'] as $id) {
                        ...
                        $this->excel->getActiveSheet()->SetCellValue('Q' . $row,strip_tags($sale->note));
                        $new_row = $row+1;
							$styleArray = array(
								'alignment' => array(
									'horizontal' => PHPExcel_Style_Alignment::HORIZONTAL_RIGHT
                            ...
                            $this->excel->getActiveSheet()->SetCellValue('J' . $new_row, $this->erp->formatMoney($sum_amount));
                            $this->excel->getActiveSheet()->SetCellValue('K' . $new_row, $this->erp->formatMoney($sum_return_sale));
                            $this->excel->getActiveSheet()->SetCellValue('L' . $new_row, $this->erp->formatMoney($sum_paid));
                            $this->excel->getActiveSheet()->SetCellValue('M' . $new_row, $this->erp->formatMoney($sum_deposit));
                            $this->excel->getActiveSheet()->SetCellValue('N' . $new_row, $this->erp->formatMoney($sum_discount));
                    ...
                    foreach ($_POST['val'] as $id) {
                        ...
                        $this->excel->getActiveSheet()->SetCellValue('H' . $row, $sale->saleman);
                        $this->excel->getActiveSheet()->SetCellValue('I' . $row, $sale->sale_status);
                        $this->excel->getActiveSheet()->SetCellValue('J' . $row, $this->erp->formatMoney($sale->amount));
                        $this->excel->getActiveSheet()->SetCellValue('K' . $row, $this->erp->formatMoney($sale->return_sale));
                        $this->excel->getActiveSheet()->SetCellValue('L' . $row, $this->erp->formatMoney($sale->paid));
                        $this->excel->getActiveSheet()->SetCellValue('M' . $row, $this->erp->formatMoney($sale->deposit));
                        $this->excel->getActiveSheet()->SetCellValue('N' . $row, $this->erp->formatMoney($sale->discount));
                        $this->excel->getActiveSheet()->SetCellValue('O' . $row, $this->erp->formatMoney($sale->balance));
                        $this->excel->getActiveSheet()->SetCellValue('P' . $row, $sale->payment_status);
                        $new_row = $row+1;
							$styleArray = array(
								'alignment' => array(
		...
		if ($rows) {
            foreach ($rows as $row) {
                ...
                if ($row->tax_rate) {
                    ...
                    $pr[] = array('id' => str_replace(".", "", microtime(true)), 'item_id' => $row->id, 'label' => $row->name . " (" . $row->code . ")" . " (" . $row->price . ")", 'row' => $row, 'combo_items' => $combo_items, 'tax_rate' => $tax_rate, 'options' => $options,'expdates'=>$expdates,'cost' => $row->cost, 'group_prices'=>$group_prices, 'all_group_price' => $all_group_prices, 'orderqty'=>$orderqty, 'makeup_cost'=>$customer_group->makeup_cost, 'customer_percent' => $customer_group->percent,'currency'=>$currency,'us_currency'=>$us_currency,'makeup_cost_percent'=>$percent->percent);
                } else {
        ...
        if ($rows) {
            foreach ($rows as $row) {
				...
				if ($row->tax_rate) {
					...
					$pr[] = array('id' => str_replace(".", "", microtime(true)), 'item_id' => $row->id, 'label' => $row->name . " (" . $row->code . ")", 'row' => $row, 'combo_items' => $combo_items, 'tax_rate' => $tax_rate, 'options' => $options,'expdates'=>$expdates,'group_prices'=>$group_prices, 'all_group_price' => $all_group_prices, 'makeup_cost'=>$customer_group->makeup_cost, 'customer_percent' => $customer_group->percent, 'makeup_cost_percent'=>$percent->percent);
					
				} else {
					$pr[] = array('id' => str_replace(".", "", microtime(true)), 'item_id' => $row->id, 'label' => $row->name . " (" . $row->code . ")", 'row' => $row, 'combo_items' => $combo_items, 'tax_rate' => false, 'options' => $options,$options,'expdates'=>$expdates,'group_prices'=>$group_prices, 'all_group_price' => $all_group_prices, 'makeup_cost'=>$customer_group->makeup_cost, 'customer_percent' => $customer_group->percent, 'makeup_cost_percent'=>$percent->percent);
				}
		...
		if ($rows) {
            foreach ($rows as $row) {
                ...
                if ($row->tax_rate) {
                    ...
                    $pr[] = array('id' => str_replace(".", "", microtime(true)), 'item_id' => $row->id, 'label' => $row->name . " (" . $row->code . ")", 'row' => $row, 'combo_items' => $combo_items, 'tax_rate' => $tax_rate, 'options' => $options, 'expdates'=>$expdates, 'group_prices'=>$group_prices, 'all_group_price' => $all_group_prices);
                } else {
	...
	function view_payment_cus($biller_id = Null, $payment_ref = Null,$idd)
    {
	...
	function invoice_st_a5($id=null)
    {
        // $this->erp->print_arrays($id);
        $this->erp->checkPermissions('add', true, 'sales');
