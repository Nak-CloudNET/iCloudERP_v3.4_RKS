...
class Purchases extends MY_Controller
{
            ...
            if (!empty($_POST['val'])) {
                if ($this->input->post('form_action') == 'delete') {
                    ...
                    if($this->Owner || $this->Admin || $this->GP['purchase_request-cost']) {
                        ...
                        $this->excel->getActiveSheet()->SetCellValue('I1', lang('noted'));
                        $row = 2;
                        $sum = 0;
                        foreach ($_POST['val'] as $id) {
                            ...
                            $this->excel->getActiveSheet()->SetCellValue('I' . $row, strip_tags($purchase->note));
    ...
    function tcs_invoice($purchase_id = null){
	 ...
	 function phum_meas_purchase_order($purchase_id = null){
        ...
        */
        ...
        $this->load->view($this->theme . 'purchases/phum_meas_purchase_order', $this->data);
    }
			...
			if($type_exp == 'po'){
				...
				if($setting->accounting_method == 2 || $shipping){
					...
					for($i = 0; $i < $c; $i++){
						...
						$costunit = $this->site->calculateAVGCost2017($array_c[$i]['product_id'], $shipping, $array_c[$i]['quantity'], $array_c[$i]['price'], $total_price, $array_c[$i]['unit_cost'], $item_discount, $this->input->post('discount'), $array_c[$i]['option_id'], $array_c[$i]['subtotal'], $total);
	...
	*********/
			...
			if($type_exp=='po'){
				...
				if($setting->accounting_method == 2 || $shipping){
					...
					for($i = 0; $i < $c; $i++){
						$item_option 	= $_POST['product_option'][$i];
						$unitCost 		= $this->erp->formatPurDecimal($_POST['unit_cost'][$i]);
						$costunit 		= $this->site->editCalculateAVGCost2017($array_c[$i]['product_id'], $shipping, $array_c[$i]['quantity'], $array_c[$i]['price'],$total_price, $array_c[$i]['unit_cost'], $item_discount, $this->input->post('discount'), $array_c[$i]['option_id'], $id, $array_c[$i]['subtotal'], $total);
    ...
    public function add_payment_old($id = null)
    {
        $this->erp->checkPermissions('payments', NULL, 'purchases');
        $this->load->helper('security');
        if ($this->input->get('id')) {
